    ...
    selectQuery: function(tableName, options) {
      ...
      if (options.include) {
        ...
        options.include.forEach(function(include) {
          ...
          if (include.association.useJunctionTable && include.association.connectorDAO && include.association.connectorDAO.__keyMap) {
            table += ", " + QueryGenerator.addQuotes(include.association.target.tableName)
            table += ' AS ' + QueryGenerator.addQuotes(include.as)

            options.table = table

            var joinQuery = " LEFT OUTER JOIN `<%= table %>` ON `<%= table %>`.`<%= attrLeft %>` = `<%= tableRight %>`.`<%= attrRight %>` AND `<%= table %>`.`<%= attrLeft2 %>` = `<%= tableRight2 %>`.`<%= attrRight2 %>`"
              , connector = include.association.connectorDAO
              , left      = Utils._.keys(connector.__keyMap)[0]
              , right     = Utils._.keys(connector.__keyMap)[1]

            query += Utils._.template(joinQuery)({
              table:      include.association.connectorDAO.tableName,
              attrLeft:    left,
              attrLeft2:   right,
              tableRight:  include.as,
              tableRight2: connector.__keyMap[right].tableName,
              attrRight:   'id',
              attrRight2:  'id'
            })
          } else {
