var Utils                = require(__dirname + '/utils')
  ...
  , DaoFactory           = require(__dirname + '/dao-factory')
...
module.exports = (function() {
  ...
  QueryInterface.prototype.showIndex = function(tableNameOrDaoFactory, options) {
    var daoFactory = ensureDaoFactory(tableNameOrDaoFactory)
      , sql        = daoFactory.indexes().toSql()

    if (this.sequelize.options.dialect === 'postgres') {
      this.sequelize.log('-----> Remove the replace hack below once a new version of node-sql has been released. <----')
      sql = sql.replace(/"/g, "'")
    }
  ...
  var ensureDaoFactory = function(tableNameOrDaoFactory) {
    if (typeof tableNameOrDaoFactory === 'string') {
      tableNameOrDaoFactory = new DaoFactory(tableNameOrDaoFactory, {}, {})
    }

    return tableNameOrDaoFactory
  }
